...
const returnParents = (categories, searchCategory, parentsArr) => {
  categories.map(category => {
    if (searchCategory.parent !== null) {
      if (searchCategory.parent.key === category.key) {
        parentsArr.push(category.name);
        returnParents(categories, category, parentsArr);
      }
    }
  });
};
exports.returnParents = returnParents;
